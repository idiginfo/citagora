...
/**
 * Class to implement the MilitarySuicideResearchConsortium (MSRC) GenericUrl
 * object
 * 
 * @author griccardi
 * 
...
public class MsrcUrl extends GenericUrl {
	...
	public MsrcUrl(MsrcApiParams params) {
		...
		if (collection == null)
			collection = MsrcApiParams.BIBLIO_COLLECTION;
		if (id == null) {
			// get all citations by timeframe
			List<String> pathParts = Arrays.asList("", collection);
			setPathParts(pathParts);
		} else {// get single document by id
			List<String> pathParts = Arrays.asList("", collection, id);
			setPathParts(pathParts);
		}
	}
