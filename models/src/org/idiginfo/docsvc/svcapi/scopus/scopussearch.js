/**
 * JavaScript functions/classes to implement access to Sciverse
 * 
 * @author griccardi
 * 
 */
...
function sciverseDebug() {
	...
	// if debuging is enabled writes the message to the end of the specified div
	// tag
	this.write = function(message) {
		if (this.getDebug()) {
			...
			document.getElementById("sciverseDebugArea")
...
function sciverseListRenderer() {
	...
	this._contentProvidedByString = " <a href=\"http://www.scopus.com\"><img src=\""
			+ this._sciverseURL
			+ "images/content_provided_by.GIF\" width=\"101\" height=\"11\" border=\"0\" /><img src=\""
			+ this._sciverseURL
			+ "images/scopus_content.GIF\" width=\"40\" height=\"11\" border=\"0\" /></a>"

	this._resultsTableStart = "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"4\" class=\"txt\">";
	this._resultsTableEnd = "<tr><td>&nbsp;</td><td>"
			+ this._contentProvidedByString + "</tr></table>"
	...
	this._documentRowStart2 = "\"><img src=\""
			+ this._sciverseURL
			+ "images/bullet.gif\" width=\"10\" height=\"12\" /></td><td valign=\"top\" class=\"";
	...
	this._inwardurlEndWhite += "&nbsp;<img style=\"vertical-align:middle\" border=0 src="
			+ sciverseURL + "images/scopus_white_new.gif></img></a>";
	this._inwardurlEndGrey = "\">View in"
	this._inwardurlEndGrey += "&nbsp;<img  style=\"vertical-align:bottom\" border=0 src="
			+ sciverseURL + "images/scopus_grey_new.gif></img></a>";
	...
	var orderedFields = [ this._title, this._doctype, this._citedbycount,
			this._inwardurl, this._sourcetitle, this._issn, this._vol,
			this._issue, this._page, this._pubdate, this._eid, this._scp,
			this._doi, this._firstAuth, this._affiliations, this._abstract,
			this._authlist ];
	...
	this.setRenderLocation = function(idName) {
		if (idName != null && idName != "") {
			...
			debug
					.write("sciverseRenderer.setRenderLocation: no render location specified");
		}
	}
	...
	this.renderErrors = function(errors) {
		...
		for (i = 0; i < errors.length; i++) {
			if (hilight) {
				output += this._errorRowStart1 + this._grey
						+ this._errorRowStart2 + this._grey
						+ this._errorRowStart3;
			} else {
				output += this._errorRowStart1 + this._white
						+ this._errorRowStart2 + this._white
						+ this._errorRowStart3;
			}

			error = errors[i].split("\n");
			output += this._errorStart + error[0] + this._errorEnd
					+ this._fieldEnd;

			if (typeof (error[1]) == 'undefined' || error[1] == null
					|| error[1] == "") {
	...
	this.getAllRenderedResultsString = function(results) {
		...
		if (results.length > 0) {
			...
			output += this._documentRowStart1 + this._white
					+ this._documentRowStart2 + this._white
	...
	this.renderDocument = function(response, position) {
		var output = this._resultsTableStart;
		output += this.getRenderedResultString(response.results[position],
				false);
		output += this._resultsTableEnd;
		this.writeString(output);
	}
	...
	this.renderField = function(response, position, field) {
		output = this.getRenderedFieldString(field,
				response.results[position][field], true);
		this.writeString(output);
	}
	...
	this.getRenderedResultString = function(result, hilight) {
		output = "";
		if (hilight) {
			output += this._documentRowStart1 + this._grey
					+ this._documentRowStart2 + this._grey
					+ this._documentRowStart3;
		} else {
			output += this._documentRowStart1 + this._white
					+ this._documentRowStart2 + this._white
		...
		for ( var i = 0; i <= orderedFields.length; i++) {
			output += this.getRenderedFieldString(orderedFields[i],
					result[orderedFields[i]], false, hilight);

		}
		/*
		 * output += this.getRenderedFieldString(this._title,
		 * result[this._title], false,hilight); for(var i in result){
		 * if(i!=this._title){ output += this.getRenderedFieldString(i,
		 * result[i], false,hilight); } }
		 */
		output += this._documentRowEnd;
		return output;
	}
	...
	this.getRenderedFieldString = function(field, string, single, hilight) {
		...
		if (field == this._title) {
			if (single) {
				output += this._titleStart + string + this._singlefieldtitle
						+ this._titleEnd;
			} else {
			...
			if (single) {
				output += this._citedbycountStart + string
			...
			if (hilight) {
				output += this._inwardurlStart + string
						+ this._inwardurlEndGrey;
			} else {
				output += this._inwardurlStart + string
						+ this._inwardurlEndWhite;
			}
...
// This class handles building and submitting the search request to the server
// as well as
...
function sciverseBackend() {
	...
	this.submitSearch = function(searchObj, apiKey, callback) {
		if (this._busy == true) {
			...
			// add in browser cacheing prevention. This is used to change the
			// generated URL
			// slightly each time to prevent the browser from caching a response
			// to identical searches.
			// That's bad because if it happens the browser won't "run" the
			// response.
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._preventCache, this.randomString());
			...
			// add in the search
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._search, searchObj.getSearch());

			// add in the sort
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._sort, searchObj.getSort());

			// add in the sortDirection
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._sortDirection, searchObj.getSortDirection());

			// add in the fields
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._fields, searchObj.getFields());

			// add in the numResults
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._numResults, searchObj.getNumResults());

			// add in the content
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._content, searchObj.getContent());

			// add in the offset
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._offset, searchObj.getOffset());

			// build the callback string
			var searchID = "search" + this._reqCounter++;
			var searchObjectString = this._callbackStringStart + searchID;
			var callbackString = this._callbackStringStart + searchID
					+ this._callbackStringEnd;
			// add in callback
			searchRequestURL = this.appendVarToURL(searchRequestURL,
					this._callback, callbackString);
			...
			setTimeout('sciverse.Backend.timeout(' + searchObjectString + ')',
					this._timeoutVal);
		}
	}
	...
	// This method is called by the inflight object when the response is
	// returned. At
	// that time it will handle reseting the state of the backend to idle, and
	// the state
	...
	this.callback = function(search, response) {
		...
		if (search.inflight == false) {
			debug
					.write("...Javascript just recieved a response for a search that had timed out.");
		} else {
	...
	// This method is called when the timer for a search runs down. If the
	// search is still active
	// at that time it resets the state of the backend to idle, marks the
	// request as complete(timedout)
