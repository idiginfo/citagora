...
public class LoadDocuments {
	...
	public Container load(Container containerFields, Document document) {
		...
		if (doi != null) {
			...
			if (ref != null) {
				...
				HarvestResult harvestResult = factory
		...
		Container container = documentMapper.createContainer(containerFields,
				document);
		HarvestResult harvestResult = factory.createHarvestResult(
	...
	/*
	 * public String writeCitagoraRdf(UriObject document, String version, int
	 * level) { MapCitagoraObject mapper = new MapCitagoraObject();
	 * mapper.add(document, level); Model model = mapper.getModel();
	 * StringWriter out = new StringWriter(); model.write(out, version); return
	 * out.toString(); }
	 */
