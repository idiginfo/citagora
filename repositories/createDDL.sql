CREATE TABLE citagora_objects (MYID INTEGER NOT NULL, DTYPE VARCHAR(31), CREATED DATETIME, GENERATED DATETIME, RIGHTS VARCHAR(255), SOURCE VARCHAR(255), TYPE VARCHAR(255), UPDATED DATETIME, URI VARCHAR(255), WASATTRIBUTEDTO VARCHAR(255), GENERATOR_MYID INTEGER, PRIMARY KEY (MYID))
CREATE TABLE CITAGORA_REFERENCES (MYID INTEGER NOT NULL, ABSTRACTTEXT VARCHAR(255), DOI VARCHAR(255), ISSUED DATETIME, LANGUAGE VARCHAR(255), OVERALLRATING DOUBLE, PAGEEND INTEGER, PAGESTART INTEGER, PAGES VARCHAR(255), PMID VARCHAR(255), PUBLISHER VARCHAR(255), SEEALSO LONGBLOB, SHORTTITLE VARCHAR(255), SUBJECT VARCHAR(255), TITLE VARCHAR(255), VOLUME VARCHAR(255), CONTRIBUTEDBY_MYID INTEGER, isPartOf INTEGER, PRIMARY KEY (MYID))
CREATE TABLE citagora.citagora_documents (MYID INTEGER NOT NULL, ISABOUT_MYID INTEGER, PRIMARY KEY (MYID))
CREATE TABLE citagora.annotations (MYID INTEGER NOT NULL, MODELVERSION VARCHAR(255), CHARACTERENCODING VARCHAR(255), CHARS VARCHAR(255), ANNOTATOR_MYID INTEGER, PRIMARY KEY (MYID))
CREATE TABLE COMMENTS (MYID INTEGER NOT NULL, RATING INTEGER, RATINGTYPE VARCHAR(255), REVIEWER_MYID INTEGER, TARGET_MYID INTEGER, PRIMARY KEY (MYID))
...
CREATE TABLE REVIEWS (MYID INTEGER NOT NULL, RATING INTEGER, RATINGTYPE VARCHAR(255), TOTALVOTES INTEGER, DOCUMENTREVIWED_MYID INTEGER, REVIEWER_MYID INTEGER, PRIMARY KEY (MYID))
...
ALTER TABLE citagora.citagora_documents ADD CONSTRAINT FK_citagora_documents_ISABOUT_MYID FOREIGN KEY (ISABOUT_MYID) REFERENCES citagora_objects (MYID)
ALTER TABLE citagora.citagora_documents ADD CONSTRAINT FK_citagora_documents_MYID FOREIGN KEY (MYID) REFERENCES citagora_objects (MYID)
...
ALTER TABLE REVIEWS ADD CONSTRAINT FK_REVIEWS_DOCUMENTREVIWED_MYID FOREIGN KEY (DOCUMENTREVIWED_MYID) REFERENCES citagora_objects (MYID)
